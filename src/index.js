...
var Wallet = function (priv, pub) {
  ...
  if (priv && !util.isValidPrivate(priv)) {
    throw new Error('Private key does not satisfy the curve requirements (ie. it is invalid)')
  }

  if (pub && !util.isValidPublic(pub)) {
...
Object.defineProperty(Wallet.prototype, 'pubKey', {
  get: function () {
    if (!this._pubKey) {
      this._pubKey = util.privateToPublic(this.privKey)
    }
    return this._pubKey
  }
})
...
Wallet.generate = function (icapDirect) {
  if (icapDirect) {
    var max = new util.BN('088f924eeceeda7fe92e1f5b0fffffffffffffff', 16)
    while (true) {
      var privKey = randomBytes(32)
      if (new util.BN(util.privateToAddress(privKey)).lte(max)) {
...
Wallet.generateVanityAddress = function (pattern) {
  ...
  while (true) {
    var privKey = randomBytes(32)
    var address = util.privateToAddress(privKey)
...
Wallet.prototype.getPrivateKeyString = function () {
  return util.bufferToHex(this.getPrivateKey())
}
...
Wallet.prototype.getPublicKeyString = function () {
  return util.bufferToHex(this.getPublicKey())
}

Wallet.prototype.getAddress = function () {
  return util.publicToAddress(this.pubKey)
}

Wallet.prototype.getAddressString = function () {
  return util.bufferToHex(this.getAddress())
}

Wallet.prototype.getChecksumAddressString = function () {
  return util.toChecksumAddress(this.getAddressString())
}
...
Wallet.prototype.toV3 = function (password, opts) {
  ...
  var mac = util.sha3(Buffer.concat([ derivedKey.slice(16, 32), Buffer.from(ciphertext, 'hex') ]))
...
Wallet.fromV1 = function (input, password) {
  ...
  var mac = util.sha3(Buffer.concat([ derivedKey.slice(16, 32), ciphertext ]))

  if (mac.toString('hex') !== json.Crypto.MAC) {
    throw new Error('Key derivation failed - possibly wrong passphrase')
  }

  var decipher = crypto.createDecipheriv('aes-128-cbc', util.sha3(derivedKey.slice(0, 16)).slice(0, 16), Buffer.from(json.Crypto.IV, 'hex'))
