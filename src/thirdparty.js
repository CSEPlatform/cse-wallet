...
Thirdparty.fromKryptoKit = function (entropy, password) {
  ...
  var privKey
  if (type === 'd') {
    privKey = util.sha256(entropy)
  } else if (type === 'q') {
    if (typeof password !== 'string') {
      throw new Error('Password required')
    }

    var encryptedSeed = util.sha256(Buffer.from(entropy.slice(0, 30)))
    ...
    */
    ...
    if (checksum.length > 0) {
      if (checksum !== util.sha256(util.sha256(privKey)).slice(0, 8).toString('hex')) {
